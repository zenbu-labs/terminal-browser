...
/// How long an idle pump may sleep under tmux before re-reading the pane size.
...
impl Engine {
    ...
    pub fn pump(&mut self, wait: Option<Duration>) -> io::Result<Vec<EngineEvent>> {
        ...
        // tmux resizes the pane without telling us in-band, so never sleep long enough to miss it
